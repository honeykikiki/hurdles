...
class FileUpload extends HTMLElement {
  ...
  render() {
    ...
    if (this.uploadedFiles != undefined) {
      this.fileVoToJson(this.uploadedFiles).forEach((file) => {
      ...
      <div>
        <div class="input-group mb-3">
          <div>
            ...
            <input type="file" name="${this.name}" data-id="${file?.fileNo}" class="form-control me-3 mr-3" value="${file?.fileOrgName || "???"}">
            <button type="button" data-id="${file?.fileNo}" class="btn btn-danger removeFileBtn">삭제</button>
          </div>
        <div>
          이미지 미리보기
          <img 
            class="col-3 rounded img-thumbnail mb-2 preview"
            src="/upload/${this.fileRoot}/${file?.fileSaveName}"
            onerror="this.src='${this.contextPath  }/assets/img/image-error.png'"/>
        </div>
      </div>
    `;
