...
<c:set var="nav" value="restaurantInsert" scope="request"/>
<c:set var="titleName" value="식당 추가" scope="request"/>
...
<body>
     ...
     <div class="container-fluid">
		 <div class="bg-white rounded p-3 m-2">
			...
			<form action="restaurantInsert.do" class="needs-validation"  method="post" enctype="multipart/form-data" novalidate>
			  <div class="mb-4">
			  ...
			  <div class="mb-4">
			    ...
			    <select class="custom-select w-50" name="restaurantCodeNo" aria-label="Large select example" required="required" value="${restaurant.restaurantCodeNo }">
				   ...
				   <c:forEach var="item" items="${resCodeList }">
  				 	  <option value="${item.restaurantCodeNo }">${item.resturantCodeName }</option>
				   </c:forEach>
			    </select>
			  ...
			  <div class="mb-4">
			    ...
			    <select class="custom-select w-50" name="localNo" aria-label="Large select example" required="required" value="${restaurant.localNo }">
				   ...
				   <c:forEach var="item" items="${localList }">
  				 	  <option value="${item.localNo }">${item.localName }</option>
				   </c:forEach>
			    </select>
...
<%-- 			  
			  <!-- 단일 파일 -->
			  <file-upload name="filename" label="식당 썸네일" max-files="1"></file-upload> --%>
			  
			  <!-- 여러개 파일 -->
			  <file-upload name="filename" label="식당 이미지" max-files="5" contextPath="${pageContext.request.contextPath  }"></file-upload>
